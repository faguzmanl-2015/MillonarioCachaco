package co.edu.unal.loteria.spi;

import static co.edu.unal.loteria.service.OfyService.factory;
...
import co.edu.unal.loteria.Constants;
...
import co.edu.unal.loteria.form.ProfileForm;
import co.edu.unal.loteria.form.ProfileForm.RolUsuario;
...
public class ConferenceApi {
    ...
    public Profile saveProfile(final User user, ProfileForm profileForm)
        ...
        RolUsuario teeShirtSize = profileForm.getTeeShirtSize();
        ...
        if (profile == null) {
            ...
            if (teeShirtSize == null) {
                teeShirtSize = RolUsuario.NOT_SPECIFIED;
            }
    ...
    private static Profile getProfileFromUser(User user) {
        ...
        if (profile == null) {
            ...
            profile = new Profile(user.getUserId(),
                    extractDefaultDisplayNameFromEmail(email), email, RolUsuario.NOT_SPECIFIED);
        }
        return profile;
    }
    ...
    public ResultadoLoteria createConference(final User user, final ResultadoForm conferenceForm)
        throws UnauthorizedException {
        if (user == null) {
            throw new UnauthorizedException("Authorization required");
        }
        ...
        final Key<ResultadoLoteria> conferenceKey = factory().allocateId(profileKey, ResultadoLoteria.class);
        ...
        ResultadoLoteria conference = new ResultadoLoteria(conferenceId, userId, conferenceForm);
    ...
    public List<ResultadoLoteria> queryConferences_nofilters() {
        // Find all entities of type Conference
        Query<ResultadoLoteria> query = ofy().load().type(ResultadoLoteria.class).order("name");
    ...
    public List<ResultadoLoteria> queryConferences(ResultadoQueryForm conferenceQueryForm) {
        Iterable<ResultadoLoteria> conferenceIterable = conferenceQueryForm.getQuery();
        List<ResultadoLoteria> result = new ArrayList<>(0);
        List<Key<Profile>> organizersKeyList = new ArrayList<>(0);
        for (ResultadoLoteria conference : conferenceIterable) {
    ...
    public List<ResultadoLoteria> filterPlayground() {
        // Query<Conference> query = ofy().load().type(Conference.class).order("name");
        Query<ResultadoLoteria> query = ofy().load().type(ResultadoLoteria.class);
    ...
    public List<ResultadoLoteria> getConferencesCreated(final User user) throws UnauthorizedException {
        ...
        return ofy().load().type(ResultadoLoteria.class)
                .ancestor(userKey)
                .order("name").list();
    }
    ...
    public ResultadoLoteria getConference(
            @Named("websafeConferenceKey") final String websafeConferenceKey)
            throws NotFoundException {
        Key<ResultadoLoteria> conferenceKey = Key.create(websafeConferenceKey);
        ResultadoLoteria conference = ofy().load().key(conferenceKey).now();
        if (conference == null) {
    ...
    public Collection<ResultadoLoteria> getConferencesToAttend(final User user)
            throws UnauthorizedException, NotFoundException {
        // If not signed in, throw a 401 error.
        if (user == null) {
            throw new UnauthorizedException("Authorization required");
        ...
        List<Key<ResultadoLoteria>> keysToAttend = new ArrayList<>();
        for (String keyString : keyStringsToAttend) {
            keysToAttend.add(Key.<ResultadoLoteria>create(keyString));
        }
